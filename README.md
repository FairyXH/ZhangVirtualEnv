...
远程数据仍通过控制端传输并写入本地测试 API，系统适配层不直接持有远程传输状态。服务端连接、采集设备在线状态和当前模拟数据均通过 WebSocket 事件与 5 秒心跳持续刷新；点击另一台采集设备后，旧设备会立即取消选中，新设备立即显示“已选择”；远程连接异常时保持 fail-open，测试环境可恢复为本地数据。
