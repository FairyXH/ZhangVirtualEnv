...
录像与快照由系统服务层核心托管。控制端只提交开始/停止/查询命令，不负责逐帧采样、传感器监听或录像写入；因此切换页面、控制端 Activity 重建或控制端进程退出不会主动中断正在进行的录像。录像帧持续写入系统数据目录，system_server 重启后会扫描未完成记录，按已写入帧数和时间范围恢复为“中断录像”，保留可查看和回放的数据。

录像采样间隔由控制端提交，范围为 0.1～300 秒。系统核心负责挂起测试数据输出、观测系统层真实数据、写入持久化帧并在停止时恢复测试环境；任何采样异常均记录日志并保持系统原行为可恢复。
