...
控制端支持连接受控的远程测试数据源，用于多设备环境采集与兼容性回归：

- 连接成功后，连接状态会立即反映在服务器列表中；
- 服务器列表中在线服务器自动置顶，在线或已连接状态使用绿色标识；设备列表中在线设备自动置顶，在线标识使用绿色显示；
- WebSocket 断开后服务器卡片显示“重连”，点击会立即创建新的连接，不会把已断开的连接继续显示为“已连接”；
- 客户端设备管理和 Token 页面会基于客户端最近心跳/连接状态显示在线或离线，而不是把启用状态误显示为可用；
- 远程环境页面使用单一“远程环境”标题，说明文字紧随标题显示；页面提供明确的“断开”操作，断开或远程开关关闭后会停止心跳、清理远程数据，并恢复本地测试环境；
- 远程数据源增加 10 秒新鲜度看门狗；检测到选中设备数据停滞或服务端 `last_data` 前进而本地快照未前进时，会自动重新发送当前订阅，相当于重新选择设备；
- 服务端按设备类型提供最新数据时间，客户端会分别检查 BLE、WiFi、Cell，避免某一种数据刷新时掩盖另一种数据源停滞；
- 设备选择列表只显示远程采集端，不显示订阅客户端；
...
- 远程 BLE、WiFi、Cell 卡片会完整保留服务端字段；页面预览最多显示前 15 条记录，并标注总数。服务器列表与采集设备列表均按每页 5 个显示，翻页不会改变远程连接状态。
...
录像由 system_server 的单一采样器持久化。控制端仅发送开始、暂停、继续和停止命令并轮询状态，前台服务重启也不会另行创建采样器。暂停会停止新增帧并显示“已暂停：名称”；继续会在同一录像会话内恢复采样。回放不会一次性解析整段录像，而是使用轻量时间轴并按小窗口懒加载帧数据，适合长时间、大帧数测试会话。回放开始时只应用一次环境基线；累计传感器事件只消费最新样本，避免长录像回放反复复制历史事件。为控制大录像的 SQLite 写入压力，帧数据仍逐帧提交，但进度元数据按固定批次更新，并在停止时完成最终时长、帧数和 finalized 标记写入。
...
测试状态与调试报告支持导出 ZIP 诊断包。诊断包包含完整后端报告、Hook 状态、当前 Profile、模块状态、设备与内存信息、最多 5,000 条内存运行日志（每 500 条切片）、崩溃栈，以及一次最多 20,000 行的 logcat 快照。logcat 在受限设备或进程中无法读取时，包内会保留错误说明而不会中断其他诊断文件导出。

地点测试数据与采集回放分开保存：采集包包含环境快照，位置轨道保存在已保存地点中。启用位置测试前必须打开模块总开关；如果地点不存在、模块关闭或坐标无效，控制端会分别报告对应原因，避免把开关状态误报为数据丢失。

### 独立检测器控制

VirEnvDetector 的“开始检测”会先初始化状态栏，再注册普通应用侧监听器和后台刷新任务；“结束”操作会停止刷新、注销监听器、结束 Hook 观测并清空本次结果。停止路径支持重复调用，便于 Activity 生命周期收尾和自动化测试。
...
模块提供受 Token 保护的 `/api/remote/status` 状态接口，用于 Android 环境测试器判断当前是否存在实际生效的远程环境模拟。接口返回远程模式、连接设备/服务端标识、更新时间，以及模块当前实际生效的 `location` 和 `env` 快照。测试器应在远程模式下将普通 App 标准 API 读数与该快照进行包含式匹配；未启用或连接未建立时回退到本地环境模拟判定。该接口仅服务于测试与兼容性验证，不改变 LSPosed 作用域和系统 Hook 架构。
## 许可证
