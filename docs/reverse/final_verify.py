#!/usr/bin/env python3
"""安装主模块（GNSS 屏蔽 + LTE 范围修复），重启，完整验证。"""
import subprocess
import time
import re
...
print("=== wait 20s ===")
time.sleep(20)
print("=== detector verdicts (grep key lines) ===")
p = subprocess.run(["adb", "logcat", "-d"], capture_output=True, timeout=120)
out = p.stdout.decode("utf-8", errors="replace")
for line in out.splitlines():
    if "VirEnvDetector" in line and any(k in line for k in [
        "location:", "cell:", "ble:", "wifi:", "sensor:", "gnss:",
    ]):
        # 去掉后续明细行
        print(line[:180])
print("=== gnss injector / lte diag ===")
for line in out.splitlines():
    if "gnss injector" in line or ("CellIdentityLte diag" in line and "got" in line):
        print(line[:200])
