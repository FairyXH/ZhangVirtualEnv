#!/usr/bin/env python3
"""Reboot 后：确认 Hook 安装 → 设置 BLE → 触发采集 → 检查诊断日志。"""
import subprocess
import time
import re
import json
...
print("\n=== 1. lspd hook 安装 ===")
print(sh("su -c 'grep -a \"ble stack hooks\" /data/adb/lspd/log/modules_*.log | tail -2'"))
...
print(curl("POST", "/api/bluetooth/set", ble_body)[:200])
time.sleep(5)
print("env ble:", curl("GET", "/api/env/status")[:250])
...
if m:
    ...
    try:
        ...
        pass
    print("\n=== 4. 采集日志（ZVirtualEnv / Bluetooth / scan） ===")
    for l in lines:
        if any(k in l for k in ["ZVirtualEnv", "BluetoothLeScanner", "ble stack", "startScan", "onScannerRegistered"]):
            print(l)
