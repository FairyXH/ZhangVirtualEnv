...
for line in out.splitlines():
    if "ZVirtualEnv" in line and any(k in line for k in ["gnss injector started", "build virtual gnss status failed", "sensor injector started", "sensor injector pending", "gnss virtual deliver"]):
        key = line[:130]
        if key not in seen:
            seen.add(key)
            print(line[:220])
