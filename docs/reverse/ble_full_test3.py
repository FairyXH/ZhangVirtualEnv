#!/usr/bin/env python3
"""Reboot 后 BLE 投递验证（buildScanResults 失败原因定位）。"""
import subprocess
import time
import re
import json
...
time.sleep(4)
# 查看实际缓存内容（含 bonded 等字段）
status = curl("GET", "/api/bluetooth/status")
print("status keys:", status[:400])
...
for mm in re.finditer(r'text="([^"]*(?:ZVE|AA:BB|RSSI|failed|error)[^"]*)"', xml3):
    print(" ", mm.group(1))
