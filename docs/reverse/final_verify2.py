#!/usr/bin/env python3
"""最终验证：GNSS Builder 类名修复 + sensor pending 修复。"""
import subprocess
import time
import re
...
print("=== detector verdicts (latest 2 rounds) ===")
verdicts = [l for l in out.splitlines() if "VirEnvDetector" in l and any(k in l for k in ["location:", "cell:", "ble:", "wifi:", "sensor:", "gnss:"])]
for line in verdicts[-12:]:
    print(line[:160])
print("=== gnss/sensor diag ===")
seen = set()
for line in out.splitlines():
    if "ZVirtualEnv" in line and any(k in line for k in ["gnss injector started", "build virtual gnss status failed", "sensor injector started", "sensor injector pending", "CellIdentityLte diag"]):
        key = line[:120]
        if key not in seen:
            seen.add(key)
            print(line[:200])
