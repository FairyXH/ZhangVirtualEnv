...
| RIL | DEVICE_VERIFIED | VERIFIED_STATIC | VERIFIED_STATIC / REQUIRES_DEVICE | Common RIL method family; exact overload dynamically matched | Target confirms `getCellInfoList(Message, WorkSource)` and `getSignalStrength(Message)`; `requestCellInfoUpdate` not assumed |
